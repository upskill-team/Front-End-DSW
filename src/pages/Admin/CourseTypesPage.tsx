...
export default function CourseTypesPage() {
  ...
  if (isLoading && courseTypes.length === 0) return <p>Cargando tipos de curso...</p>;
  if (error) return <p className="text-center p-8 text-red-600">Error al cargar los datos: {error.message}</p>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button size="md" onClick={openModalForCreate}><Plus className="w-4 h-4 mr-2" />Crear Nuevo Tipo</Button>
      </div>
            ...
            {courseTypes.map((type) => (
              <div key={type.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-lg border hover:bg-slate-50/50">
                <div className="flex-1 mb-3 sm:mb-0">
                  ...
                  <p className="text-xs text-slate-500 mt-1">{(type.courses || []).length} cursos asociados</p>
                </div>
                <div className="flex items-center space-x-2 self-end sm:self-center">
                  <Button variant="outline" size="md" onClick={() => openModalForEdit(type)}><Edit className="w-4 h-4 mr-1" /> Editar</Button>
                  <Button variant="destructive" size="md" onClick={() => handleDelete(type.id)} disabled={deleteMutation.isPending}><Trash2 className="w-4 h-4 mr-1" /> Eliminar</Button>
                </div>
              </div>
            ))}
          </div>
          {courseTypes.length === 0 && !isLoading && <p className="text-center text-slate-500 py-8">No se encontraron tipos de curso con los filtros aplicados.</p>}
          
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6 pt-4 border-t">
              <Button variant="outline" onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1 || isLoading}>Anterior</Button>
              <span className="text-sm text-slate-600">Página {currentPage} de {totalPages}</span>
              <Button variant="outline" onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages || isLoading}>Siguiente</Button>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogHeader><DialogTitle>{currentTypeId ? 'Editar' : 'Crear'} Tipo de Curso</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 pt-4">
          <Input id="name" label="Nombre del Tipo de Curso" {...register('name')} error={errors.name?.message} />
          <Textarea id="description" label="Descripción" {...register('description')} error={errors.description?.message} />
          <div className="flex justify-end gap-2 pt-2">
            <Button type="button" variant="outline" size="md" onClick={() => setIsModalOpen(false)}>Cancelar</Button>
            <Button type="submit" size="md" isLoading={createMutation.isPending || updateMutation.isPending}>Guardar</Button>
          </div>
        </form>
      </Dialog>
    </div>
