...
import { Check, X, Mail, GraduationCap, FileText, Search, Clock, ExternalLink } from 'lucide-react';
...
export default function ProfessorRequestsPage() {
  ...
  const handleStatusChange = (requestId: string, newStatus: 'accepted' | 'rejected') => {
    updateAppeal({ appealId: requestId, payload: { state: newStatus } });
  };
  
  if (isLoading && requests.length === 0) return <div className="text-center p-8">Cargando solicitudes...</div>;
  if (error) return <div className="text-center p-8 text-red-600">{error.message}</div>;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-800 mb-2">Solicitudes de Profesores</h1>
        <p className="text-slate-600">Gestiona las solicitudes para convertirse en profesor</p>
      </div>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 w-full lg:w-auto">
              ...
              <div className="flex gap-2 w-full">
                ...
                <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="flex-1 w-full px-3 py-2 text-sm border rounded-lg bg-white/80 border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50">
        ...
        <CardContent>
          <div className="space-y-4">
            {requests.map((request) => {
              ...
              const formattedDate = !isNaN(date.getTime()) ? date.toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' }) : 'Fecha inválida';
                ...
                <div key={request.id} className="p-4 rounded-lg border border-slate-200 space-y-4">
                  <div className="flex flex-col lg:flex-row justify-between lg:items-start gap-4">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start gap-2 w-full lg:w-auto">
                      <Button variant="outline" size="md" onClick={() => setSelectedRequest(request)}><FileText className="w-4 h-4 mr-2" />Detalles</Button>
                      {request.state === 'pending' && (
                        <div className="flex gap-2 w-full">
                          <Button size="md" className="flex-1" onClick={() => handleStatusChange(request.id, 'accepted')} disabled={isUpdating} isLoading={isUpdating}><Check className="w-4 h-4 mr-2" />Aprobar</Button>
                          <Button variant="destructive" size="md" className="flex-1" onClick={() => handleStatusChange(request.id, 'rejected')} disabled={isUpdating} isLoading={isUpdating}><X className="w-4 h-4 mr-2" />Rechazar</Button>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {requests.length === 0 && !isLoading && (
            <p className="text-center text-slate-500 py-8">No se encontraron solicitudes con los filtros aplicados.</p>
          )}

          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6 pt-4 border-t">
              <Button variant="outline" size="md" onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1 || isLoading}>Anterior</Button>
              <span className="text-sm text-slate-600">Página {currentPage} de {totalPages}</span>
              <Button variant="outline" size="md" onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages || isLoading}>Siguiente</Button>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={!!selectedRequest} onOpenChange={() => setSelectedRequest(null)}>
        {selectedRequest && (
          <>
            <DialogHeader><DialogTitle>Detalles de la Solicitud</DialogTitle></DialogHeader>
            <div className="space-y-4 mt-4">
              ...
              {selectedRequest.documentUrl && (
                <div>
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setDocumentUrlToShow(selectedRequest.documentUrl!)} className="inline-flex items-center text-blue-600 hover:text-blue-800 hover:underline text-sm font-medium p-0">
      ...
      {documentUrlToShow && <DocumentViewer url={documentUrlToShow} onClose={() => setDocumentUrlToShow(null)} />}
    </div>
  );
}