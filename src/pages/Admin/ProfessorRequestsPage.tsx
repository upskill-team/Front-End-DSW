...
import { Check, X, Mail, GraduationCap, FileText, Search, Clock } from 'lucide-react';
...
export default function ProfessorRequestsPage() {
  ...
  const filteredAndSortedRequests = useMemo(() => {
    return requests
      .filter(req => {
        const matchesSearch = `${req.user.name} ${req.user.surname}`.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleStatusChange = async (requestId: string, newStatus: 'accepted' | 'rejected') => {
    ...
    setRequests(prev =>
      prev.map(req => (req.id === requestId ? { ...req, state: newStatus } : req))
    );
    ...
    try {
      ...
      setRequests(prev =>
        prev.map(req => (req.id === requestId ? originalRequest : req))
      );
    }
  };

  if (isLoading) return <div className="text-center p-8">Cargando solicitudes...</div>;
      ...
      <Card>
        ...
        <CardContent>
              ...
              >
                <div className="flex-1 space-y-2 mb-4 sm:mb-0">
                  <div className="flex items-center flex-wrap gap-x-3 gap-y-2">
                    <h3 className="font-semibold text-slate-800">{request.user.name} {request.user.surname}</h3>
                ...
                <div className="hidden sm:flex items-center space-x-2">
                  ...
                  {request.state === 'pending' && (
                    <>
                      ...
                      <Button variant="destructive" onClick={() => handleStatusChange(request.id, 'rejected')}>
                ...
                <div className="flex sm:hidden flex-col gap-2 w-full">
                    ...
                    {request.state === 'pending' && (
                      <div className="flex gap-2">
                        <Button className="flex-1" onClick={() => handleStatusChange(request.id, 'accepted')}><Check className="w-4 h-4 mr-1" />Aprobar</Button>
                        <Button variant="destructive" className="flex-1" onClick={() => handleStatusChange(request.id, 'rejected')}><X className="w-4 h-4 mr-1" />Rechazar</Button>
                      </div>
                    )}
                  </div>
              </div>
          ...
          {filteredAndSortedRequests.length === 0 && (
            <p className="text-center text-slate-500 py-8">No se encontraron solicitudes que coincidan con la búsqueda.</p>
          )}
        </CardContent>
      </Card>
          ...
          <>
            ...
            <div className="space-y-4 mt-4">
              ...
              {selectedRequest.documentUrl && (
                 <div>
                    <p className="text-sm font-medium text-slate-700">Documento Adjunto</p>
                    <a href={selectedRequest.documentUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                      Ver documento
                    </a>
                 </div>
              )}
