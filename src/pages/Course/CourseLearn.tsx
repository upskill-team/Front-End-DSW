...
export default function CourseLearn() {
  ...
  // Hook para marcar unidades como completadas
  const { completeUnit, isPending: isCompletingUnit } = useCompleteUnit();

  // Si no hay inscripción (404), redirigir a la página de detalles del curso
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50">
      ...
      <div className="flex flex-col lg:flex-row lg:h-[calc(100vh-88px)]">
        {/* Sidebar izquierdo - Lista de unidades - Oculto en móvil por defecto */}
        ...
        <div className="flex-1 overflow-y-auto bg-gradient-to-br from-white via-slate-50 to-blue-50">
          {currentUnit ? (
            <div className="p-4 sm:p-6 md:p-8 max-w-5xl mx-auto">
              ...
              {/* Encabezado de la unidad - Responsive */}
              ...
              {/* Contenido de la unidad (BlockNote) - Responsive */}
                    ...
                    fallback={
                      <div className="text-center py-8 text-slate-600">
                        ...
                        <p className="text-xs sm:text-sm mt-2">
                          Por favor, contacta al instructor si el problema
                          persiste.
                        </p>
                      </div>
                    }
              ...
              {/* Botón para marcar como completada - Responsive */}
                ...
                {isUnitCompleted ? (
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl p-4 sm:p-6 text-center shadow-sm">
                    ...
                    <p className="text-sm sm:text-base text-green-700 font-medium mb-1">
                      Has terminado esta unidad exitosamente
                    </p>
                    <p className="text-xs sm:text-sm text-green-600">
                      Continúa con la siguiente unidad para seguir aprendiendo.
                    </p>
                  </div>
                ) : (
                  <div className="bg-white/80 backdrop-blur-sm rounded-xl border-2 border-blue-200 p-4 sm:p-6 shadow-sm">
                    <div className="text-center mb-4">
                      <h3 className="text-base sm:text-lg font-semibold text-slate-800 mb-2">
                        ¿Completaste esta unidad?
                      </h3>
                      <p className="text-xs sm:text-sm text-slate-600">
                        Marca esta unidad como completada para actualizar tu
                        progreso
                      </p>
                    </div>
                    <Button
              ...
              <div className="text-center max-w-md bg-white/80 backdrop-blur-sm rounded-xl p-6 sm:p-8 border border-slate-200 shadow-sm">
                ...
                <p className="text-sm sm:text-base text-slate-600 mb-4">
                  <span className="hidden lg:inline">
                    Selecciona una unidad del menú lateral para comenzar a ver
                    el contenido del curso
                  </span>
                  <span className="lg:hidden">
                    Usa el selector de arriba para elegir una unidad
                  </span>
                </p>
                <div className="text-xs sm:text-sm text-slate-500">
                  {enrollment.course.units?.length > 0 ? (
                    <span>
                      📚 {enrollment.course.units.length} unidades disponibles
                    </span>
                  ) : (
