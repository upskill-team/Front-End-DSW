...
export default function TakeAssessmentPage() {
  ...
  const handleStartAttempt = async () => {
    if (!enrollmentId || !assessmentId) return;
  ...
  useEffect(() => {
    if (hasStarted && assessment?.durationMinutes && timeLeft === null) {
      setTimeLeft(assessment.durationMinutes * 60); // Convert to seconds
    }
  ...
  if (!hasStarted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-slate-50 flex items-center justify-center p-6">
      ...
      <div className="container mx-auto px-6 py-8 max-w-4xl">
        <div className="space-y-6">
          {assessment.questions?.map((question, index) => {
            if (!question.id) return null; // Skip questions without ID
                  ...
                  {question.type === 'multiple_choice' ? (
                    <div className="space-y-3">
                      {question.options?.map((option) => (
                        <label
                          key={option.id}
                          className="flex items-center gap-3 p-4 rounded-lg border-2 border-slate-200 hover:border-purple-300 hover:bg-purple-50 cursor-pointer transition-all"
                        >
                          <input
                            type="radio"
                            ...
                            checked={
                              question.id
                                ? answers[question.id] === Number(option.id)
                                : false
                            }
                            onChange={() =>
                              handleAnswerChange(
