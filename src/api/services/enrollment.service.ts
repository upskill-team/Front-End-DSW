 ...
 */
const enrollInCourse = async (payload: EnrollInCoursePayload): Promise<Enrollment> => {
 ...
 */
const findByStudentAndCourse = async ({ studentId, courseId }: { studentId: string; courseId: string }): Promise<Enrollment | null> => {
    ...
    try {
        const response = await apiClient.get<ApiResponse<Enrollment>>( `/enrollments/student/${studentId}/course/${courseId}`);
        return response.data.data;
    } catch (error) {
...
const findById = async (enrollmentId: string): Promise<Enrollment> => {
    const response = await apiClient.get<ApiResponse<Enrollment>>(`/enrollments/${enrollmentId}`);
    return response.data.data;
}
 ...
 */
const update = async (enrollmentId: string, payload: UpdateEnrollmentPayload): Promise<Enrollment> => {
    const response = await apiClient.patch<ApiResponse<Enrollment>>(`/enrollments/${enrollmentId}`, payload);
    return response.data.data;
}
